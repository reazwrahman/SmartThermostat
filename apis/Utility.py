...
class Utility: 
    ...
    def record_state_transition(self, status: bool, effective_temperature:float): 
        ...
        if payload["last_turned_on"]:  
            last_turned_on:datetime = datetime.datetime.strptime(payload["last_turned_on"], "%Y-%m-%d %H:%M:%S.%f")
            payload["on_for_minutes"] = round((payload["current_timestamp"] - last_turned_on).total_seconds() / 60,2) 
        if payload["last_turned_off"]:  
            last_turned_off:datetime = datetime.datetime.strptime(payload["last_turned_off"], "%Y-%m-%d %H:%M:%S.%f")
            payload["off_for_minutes"] = round((payload["current_timestamp"] - last_turned_off).total_seconds() / 60,2) 
...
if __name__ =="__main__": 
    ...
    assert type(utility.get_safety_configs(DeviceConfigKeys.MAXIMUM_ON_TIME.value)) == int, "Safety configs values are not loaded properly" 
    assert type(utility.get_safety_configs(DeviceConfigKeys.COOL_DOWN_PERIOD.value)) == int, "Safety configs values are not loaded properly" 
    assert type(utility.get_safety_configs(DeviceConfigKeys.MINIMUM_ON_TIME.value)) == int, "Safety configs values are not loaded properly" 
    ...
    assert os.path.exists(os.path.join(os.getcwd(), STATE_CHANGE_LOGGER)) == True, "State transition record didn't get created or got deleted prematuredly" 
    ...
    assert os.path.exists(os.path.join(os.getcwd(), STATE_CHANGE_LOGGER)) == False, "State transition record didn't get deleted as expected" 
    print ("all passed")